...
def plot_venus_position(venus_times,venus_positions):
    ...
    for this_time,this_position in zip(venus_times,venus_positions):
        plt.scatter(this_position.ra.to('deg'),this_position.dec.to('deg'),s=10,marker = 'o')
    plt.xlabel('RA')
    plt.ylabel('Dec')
    plt.title('Venus position in the sky between',venus_times[0], ' and the ',venus_times[-1], 'in the ICRS frame')
# def search_hek():
#     # todo : define this function
#     return 0
